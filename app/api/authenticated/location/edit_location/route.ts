...
export async function POST(req: NextRequest) {
  const { location_id, location_name, branch } = await req.json();
 ...
 let branch_id = '0';
   if(!branch || branch == "No Branch"){
     try{
       ...
       const[b_rows,b_fields] = await connection.execute<Branch[]>(b_query)
       if(b_rows.length==1){
  ...
  try{
   ...
   const [test_rows, test_fields] = await connection.execute<Test[]>(test_query,
   [location,
    branch_id]
   );
